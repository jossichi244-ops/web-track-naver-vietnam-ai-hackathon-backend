...
async def create_comment_route(
    ...
    return await task_comment_controller.create_comment(req.dict(), request, user)
...
async def list_comments_route(
    ...
    return await task_comment_controller.list_comments(task_id)
...
async def get_comment_route(
    ...
    return await task_comment_controller.get_comment(comment_id)


@router.put("/{comment_id}", response_model=TaskCommentResponse)
async def update_comment_route(
    comment_id: str,
    req: TaskCommentUpdate,
    request: Request,
    user=Depends(get_current_user)
):
    return await task_comment_controller.update_comment(
        comment_id,
        req.dict(),
        request,
        user
...
async def delete_comment_route(
    ...
    return await task_comment_controller.delete_comment(
        comment_id,
        request,
        user
    )
